...
const postCtrl = {
  newPost: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ success: true, msg: "post created", post: newPost });
    } catch (err) {
  ...
  getSinglePost: async (req, res) => {
    try {
      ...
      // ProfileOwner Data
      const postID = req.params.id;
      const postOwnerProfileID = await Post.findById(postID, {
        ownerProfileID: 1,
      });
      const ownerProfile = await Profile.findById(
        postOwnerProfileID.ownerProfileID,
      ...
      const GetAccess = () => {
        if (
          ...
          ownerProfile.accountType === "public" ||
          ownerProfile._id.equals(userProfile._id)
        ) {
          return (Access = true);
        } else {
          return (Access = false);
      ...
      if (Access) {
        ...
        res
          .status(401)
          .json({ success: false, Msg: "you can not access this post" });
      }
    } catch (error) {
  ...
  getFeed: async (req, res) => {
    try {
      ...
      for (i = 0; i < Feed.length; i++) {
        const singlePost = await getPost(Feed[i], userProfileID).then(function (
          post
        ) {
          return post;
        });
